@echo off
rem 此批处理文件用于修复ZLIB和googletest问题并构建Kratos Core
...
set EXTERNAL_LIBS=%KRATOS_SOURCE%\external_libraries
...
rem 修复ZLIB CMakeLists.txt文件
echo 修复ZLIB CMake配置...
if exist "%EXTERNAL_LIBS%\zlib\CMakeLists.txt" (
    echo 备份原始ZLIB CMakeLists.txt...
    copy "%EXTERNAL_LIBS%\zlib\CMakeLists.txt" "%EXTERNAL_LIBS%\zlib\CMakeLists.txt.bak"
    
    echo 更新ZLIB CMake版本要求...
    powershell -Command "(Get-Content '%EXTERNAL_LIBS%\zlib\CMakeLists.txt') -replace 'cmake_minimum_required\(VERSION [0-9]+\.[0-9]+\)', 'cmake_minimum_required(VERSION 3.5)' | Out-File '%EXTERNAL_LIBS%\zlib\CMakeLists.txt' -Encoding ASCII"
)
