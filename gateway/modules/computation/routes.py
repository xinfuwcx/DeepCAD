...
async def run_solver_task(job_id: UUID, client_id: str):
    """
    This is the background task that simulates a long-running computation.
    """
    # Update job status to running
    jobs[job_id].status = JobStatus.RUNNING
    jobs[job_id].started_at = datetime.utcnow()

    # Simulate solver steps and send progress
    try:
        await manager.send_personal_message(json.dumps({
            "status": "processing", "progress": 0, "message": f"Job {job_id} started. Initializing solver..."
        }), client_id)
        await asyncio.sleep(2)

        for i in range(1, 11):
            await manager.send_personal_message(json.dumps({
                "status": "processing", "progress": i * 10, "message": f"Computation step {i}/10 finished."
            }), client_id)
        ...
        # In a real scenario, this would be a URL to the result file (e.g., VTK, HDF5)
        jobs[job_id].result_url = f"/results/dummy_result_{job_id}.vtk"
        
        await manager.send_personal_message(json.dumps({
            "status": "completed", "progress": 100, "message": "Computation finished successfully.",
