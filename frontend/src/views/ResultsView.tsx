/**
 * ResultsView.tsx - CAE后处理结果查看器
 * 
 * 功能描述:
 * - 专业的CAE仿真结果可视化界面
 * - 支持多种后处理功能：云图显示、切片分析、变形动画
 * - 集成现代化3D渲染引擎和交互工具
 * - 适用于深基坑工程的分析结果展示和评估
 * 
 * 后处理功能:
 * 1. 云图显示 - 应力、应变、位移、温度、压力等物理量的颜色映射
 * 2. 切片分析 - 沿指定方向的结果截面查看
 * 3. 变形动画 - 结构变形过程的动态演示
 * 4. 数据探查 - 节点/单元数据的详细查看
 * 
 * 可视化控制:
 * - 多种颜色映射方案 (rainbow, hot, cool, viridis, plasma)
 * - 透明度和等级数调节
 * - 变形比例缩放
 * - 动画播放控制
 * 
 ...
 * - 截图和导出功能
 * 
 * 技术栈: React + Three.js + WebGL + 专业CAE后处理库
 */
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try {
        ...
        console.warn('[ResultsView] cleanup warning:', e);
      }
    };
  }, [animationState]);
