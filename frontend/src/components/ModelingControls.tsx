...
import { useSceneStore, Point3D, Excavation, Tunnel } from '../stores/useSceneStore';
...
const ModelingControls: React.FC<ModelingControlsProps> = ({ onShowMaterialLibrary }) => {
    const { scene, fetchScene, uploadBoreholes, createExcavationFromDXF, recalculateBoundingBox, createParameterizedTunnel, assignMaterial } = useSceneStore();
    ...
    const handleCreateTunnel = () => {
        ...
        createParameterizedTunnel(tunnelName, path, tunnelRadius);
    ...
    const handleAssignMaterial = (componentType: 'excavation' | 'tunnel', componentId: string, materialId: string | null) => {
        assignMaterial(componentType, componentId, materialId);
    ...
    return (
        <Space direction="vertical" size="middle" style={{width: '100%'}}>
            ...
            <List
              ...
              bordered
              dataSource={[...scene.excavations, ...scene.tunnels]}
              renderItem={(item) => {
