...
def test_basic_kratos_interface():
    ...
    try:
        ...
        print("OK KratosInterface created successfully")
        ...
        if interface.setup_model(test_data):
            ...
            print("OK Model setup failed")
            return False
            
    except Exception as e:
        print(f"OK Basic interface test failed: {e}")
        return False
...
def test_advanced_solver_standalone():
    ...
    try:
        ...
        print("OK Solver optimization successful")
        ...
        print(f"OK Advanced solver test failed: {e}")
        return False
...
def test_parallel_optimizer_standalone():
    ...
    try:
        ...
        print(f"OK System profiling successful")
        ...
        print("OK Parallel optimization successful")
        ...
        print(f"OK Parallel optimizer test failed: {e}")
        return False
...
def test_fpn_parsing():
    ...
    try:
        ...
        print("OK FPN parser created successfully")
        ...
        for test_file in test_files:
            if os.path.exists(test_file):
                ...
                try:
                    ...
                    print(f"OK FPN parsing successful")
                    print(f"   Nodes: {len(result.get('nodes', []))}")
                    print(f"   Elements: {len(result.get('elements', []))}")
                    print(f"   Materials: {len(result.get('materials', {}))}")
                    return True
                except Exception as e:
                    print(f"OK FPN parsing failed: {e}")
                    continue
        
        print("OKOK  No valid FPN test files found")
        return True  # 不算失败，因为文件可能不存在
        
    except Exception as e:
        print(f"OK FPN parsing test failed: {e}")
        return False
...
def main():
    ...
    for test_name, test_func in tests:
        try:
            ...
            print(f"OK {test_name} exception: {e}")
    ...
    for test_name, result in results.items():
        status = "OK PASS" if result['success'] else "OK FAIL"
